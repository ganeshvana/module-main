...
class SaleOrder(models.Model):
    ...
    def create_amendment(self):
        ...
        view_ref = self.env.ref('sale.view_order_form').id
...
#             {'sale_line_id': True})
    ...
    def copy(self, defaults=None):
        ...
        if self.env.context.get('new_sale_amendment'):
            ...
            print(type(revno),revno)
            print(type(self.amendment_name),self.amendment_name)
            # Assign default values for views
            self.write({'amendment_no': str(revno + 1), 'name': str(self.name) + '-' + '%s-%02d' % ('AMD', int(revno + 1))})
            defaults.update({'name': prev_name,'amendment_no': revno,'state': 'cancel','invoice_count': 0,'current_amendment_id': self.id,'amendment_name': self.name})
